...
<div class="content-wrapper">
    ...
    <tbody>
           @foreach($students as $st)
            ...
            <td>
              @if($st->image)
                  <img src="{{ asset('storage/' . $st->image) }}" alt="Student Image" style="width: 50px; height: 50px; border-radius: 50%;">
              @else
          ...
          <button class="btn btn-info" onclick="document.getElementById('upload_image_{{ $st->id }}').click()">Upload Image</button>
          <form id="upload_form_{{ $st->id }}" action="{{ route('students.upload_image', $st->id) }}" method="POST" enctype="multipart/form-data" style="display:none;">
              @csrf
              <input type="file" id="upload_image_{{ $st->id }}" name="image" onchange="document.getElementById('upload_form_{{ $st->id }}').submit();">
          </form>
        </td>
            </tr>
            @endforeach
